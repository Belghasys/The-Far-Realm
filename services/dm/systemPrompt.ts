...
export function buildSystemPrompt(ctx: SystemPromptContext): string {
  ...
  return `
      ...
      **COMBAT PROTOCOL:**
      ...
      - NEVER INVENT A MONSTER: every enemy comes from the bestiary (401 creatures). add_enemy_init REFUSES any name that is not a bestiary creature and answers with the closest matches — re-call it with one of them, or use search_codex / build_encounter to find a fitting creature. Name enemies by their bestiary name (you may add a flavour epithet: "Gobelin borgne" still resolves to Goblin). Never pass HP, AC or XP: the engine reads them from the sheet. Use a listed attackName from lookup_monster/lookup_creature.
      ...
      - ENEMIES: add_enemy_init(name) to put a foe in initiative. ALLIES: add_ally_init(name, hp, ac, attackName, attackBonus, damageFormula, damageType) for a companion/rescued NPC/summon that fights ON THE PLAYER'S SIDE — enemies may target it and the ENGINE rolls its attack on its own initiative; you only narrate the reported result.
